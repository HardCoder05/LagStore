...
public class DesarrolladorWS {
    ...
    @WebMethod(operationName = "registroDesarrollador")
    public int registroDesarrollador(Desarrollador desarrollador){
        try{
            boDesarrollador = new DesarrolladorBO();
            return boDesarrollador.insertar(desarrollador);
        }catch(Exception e){
            throw new WebServiceException("Error al insertar desarrollador: " + e.getMessage());
        }
    }
}
